...
class BasketballShotCounter {
        ...
        this.players.forEach(player => {
            ...
            card.innerHTML = `
                <div class="player-header" style="margin-bottom: 10px;">
                    <h3 style="font-size: 18px; margin: 0;">${playerDisplayName}</h3>
                    <button class="delete-btn" onclick="app.deletePlayer('${player.id}')" aria-label="Delete player">×</button>
                </div>
                <div class="player-actions">
                    <button class="btn btn-blue" onclick="app.openStatsModal('${player.id}')" style="width: 100%; padding: 10px; font-size: 14px;">View Stats</button>
                </div>
            `;
    ...
    renderCourt() {
        ...
        let totalShots = 0;
        // Add shot markers for all players
        this.players.forEach(player => {
    ...
    renderDetailedStats(player) {
        ...
        this.shotTypes.forEach(type => {
            const typeStats = stats.shotTypeCounts[type] || { total: 0, made: 0, missed: 0 };
            const percentage = typeStats.total > 0 ? ((typeStats.made / typeStats.total) * 100).toFixed(1) : '0';
            
            html += `
                <div class="shot-type-stat-item">
                    <div class="shot-type-name">${typeLabels[type]}</div>
                    <div class="shot-type-numbers">
                        <span>Total: ${typeStats.total}</span>
                        <span>Made: ${typeStats.made}</span>
                        <span>Missed: ${typeStats.missed}</span>
